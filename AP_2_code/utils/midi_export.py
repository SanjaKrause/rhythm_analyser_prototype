...
def onsets_to_midi(
    ...
    for onset_time in onset_times:
        ...
        last_tick = tick_time + duration_ticks
...
def f0_to_midi(
    ...
    for note_dict in notes:
        ...
        track.append(Message('note_on', note=note_num, velocity=DEFAULT_VELOCITY, time=delta_time))

        # Note off
        track.append(Message('note_off', note=note_num, velocity=0, time=duration_ticks))
...
def comprehensive_csv_to_onset_midi(
    ...
    for method in methods:
        ...
        output_file = output_dir / f"{method}.mid"
        try:
            midi_path = onsets_to_midi(onset_times_subset, str(output_file), tempo=avg_tempo)
            midi_files.append(midi_path)
