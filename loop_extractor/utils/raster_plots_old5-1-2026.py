        ...
        for _, ref in method_refs.iterrows():
            ...
            if 0 <= bar_idx < n_bars:
                ...
                if is_new_method and bar_duration is not None and bar_duration > 0:
                    ...
                    # grid_phase = where 1/16 position is in uncorrected grid (should be 1/16 = 0.0625)
                    # After correction: grid shifts by ref_ms so that grid_phase (1/16) aligns with onset

                    ref_s = ref_ms / 1000.0

                    # Red circle: Where the onset IS after correction (should be at 0ms = at tick 1 = 1/16)
                    # The target tick is 1, which in display coordinates is 1/16
                    red_phase_corrected = 1.0 / steps_per_bar  # = 1/16

                    # Pink circle: Where 1/16 WAS before correction (in corrected coordinates)
                    # Before correction, 1/16 was at grid_phase in uncorrected coordinates
                    # In corrected coordinates: grid_phase - (ref_s / bar_duration)
                    # But wait - we need to think about this differently:
                    # The grid_phase position hasn't moved in absolute time
                    # But we're measuring in the NEW shifted grid coordinate system
                    # So: pink_corrected = grid_phase - (ref_s / bar_duration)
                    pink_phase_corrected = grid_phase - (ref_s / bar_duration)
                    pink_phase_shifted = pink_phase_corrected + phase_shift

                    ax.scatter([red_phase_corrected], [bar_idx],
                               s=90, facecolors='none', edgecolors='red',
                               linewidths=1.5, marker='o', zorder=10)

                    ax.scatter([pink_phase_shifted], [bar_idx],
                    ...
                    if ref_ms is not None and np.isfinite(ref_ms):
                        label_x = max(red_phase_corrected, pink_phase_shifted) + 0.02
                    ...
                    # grid_phase = where grid position should be
...
def create_new_grid_corrections_plot(
    csv_file: str,
    output_file: str,
    track_id: str,
    rms_values: Optional[dict] = None
    ...
    """
    Create 6-panel raster plot showing new grid correction methods.

    Panels:
    1. Uncorrected
    2. Per-snippet (old)
    3. Pattern length L=4 (old)
    4. Per-snippet (new)
    5. 4-bar (new)
    6. 4-bar loopstart (new)
    ...
    Examples
    --------
    >>> create_new_grid_corrections_plot(
    ...     'track_comprehensive_phases.csv',
    ...     'track_new_grid_corrections.png',
    ...     'track_123'
    ... )
    """
    # Load data
    ...
    fig = plt.figure(figsize=(12, fig_height * 3.0))
    ...
    rms_uncorr = rms_values.get('uncorrected_ms') if rms_values else None
    rms_per_snip = rms_values.get('per_snippet_ms') if rms_values else None
    rms_standard_l4 = rms_values.get('standard_L4_ms') if rms_values else None
    rms_new_per_snip = rms_values.get('new_per_snippet_ms') if rms_values else None
    rms_new_4bar = rms_values.get('new_4bar_ms') if rms_values else None
    rms_new_4bar_loopstart = rms_values.get('new_4bar_loopstart_ms') if rms_values else None
    ...
    # Plot 2: Per-snippet (old)
    plot_raster_single(
        axes[1], df, 'phase_per_snippet_remapped',
        'Per snippet (old)', track_id, rms_ms=rms_per_snip,
        ref_onsets=ref_onsets, method_name='per_snippet'
    )

    # Plot 3: Pattern length L=4 (old)
    standard_l4_col = 'phase_standard_L4(L=4)'
    if standard_l4_col in df.columns:
        plot_raster_single(
            axes[2], df, standard_l4_col,
            'Pattern length L=4 (old)', track_id, rms_ms=rms_standard_l4,
            ref_onsets=ref_onsets, method_name='standard_L4(L=4)'
        )
    else:
        axes[2].text(0.5, 0.5, 'Pattern length L=4 not available',
                     ha='center', va='center', transform=axes[2].transAxes)
        axes[2].set_title('Pattern length L=4 (old)')

    # Plot 4: Per-snippet (new)
    if 'phase_new_per_snippet' in df.columns:
        plot_raster_single(
            axes[3], df, 'phase_new_per_snippet',
            'Per snippet (new)', track_id, rms_ms=rms_new_per_snip,
            ref_onsets=ref_onsets, method_name='new_per_snippet'
        )
    else:
        axes[3].text(0.5, 0.5, 'New per-snippet not available',
                     ha='center', va='center', transform=axes[3].transAxes)
        axes[3].set_title('Per snippet (new)')

    # Plot 5: 4-bar (new)
    new_4bar_col = 'phase_new_4bar(L=4)'
    if new_4bar_col in df.columns:
        plot_raster_single(
            axes[4], df, new_4bar_col,
            '4-bar (new)', track_id, rms_ms=rms_new_4bar,
            ref_onsets=ref_onsets, method_name='new_4bar(L=4)'
        )
    else:
        axes[4].text(0.5, 0.5, '4-bar (new) not available',
                     ha='center', va='center', transform=axes[4].transAxes)
        axes[4].set_title('4-bar (new)')

    # Plot 6: 4-bar loopstart (new)
    new_4bar_loopstart_col = 'phase_new_4bar_loopstart(L=4)'
    if new_4bar_loopstart_col in df.columns:
        plot_raster_single(
            axes[5], df, new_4bar_loopstart_col,
            '4-bar loopstart (new)', track_id, rms_ms=rms_new_4bar_loopstart,
            ref_onsets=ref_onsets, method_name='new_4bar_loopstart(L=4)'
        )
    else:
        axes[5].text(0.5, 0.5, '4-bar loopstart (new) not available',
                     ha='center', va='center', transform=axes[5].transAxes)
        axes[5].set_title('4-bar loopstart (new)')

    plt.tight_layout()
    plt.savefig(output_file, dpi=150, bbox_inches='tight')
    plt.close()

    print(f"  ✓ New grid corrections plot saved: {output_file}")
...
def create_all_plots(
    ...
    # Create 6-panel new grid corrections plot
    print("  Creating new grid corrections plot...")
    create_new_grid_corrections_plot(
        csv_file,
        str(output_dir / f"{track_id}_new_grid_corrections.png"),
        track_id,
        rms_values=rms_values
    )
