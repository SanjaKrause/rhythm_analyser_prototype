...
def find_reference_offset_new_per_snippet(
    downbeats: List[float],
    onsets: np.ndarray,
    first_bar: int,
    steps_per_bar: int,
    max_match_frac: float = MAX_MATCH_FRAC,
    search_window_start_phase: float = 0.5,
    search_window_end_phase: float = 0.75
) -> Tuple[float, int, float, float]:
    """
    Find reference offset using NEW per-snippet method.

    Changes from original per_snippet:
    - ONLY checks position 1/16th (tick 1) - NO fallback positions
    - Uses configurable search window relative to 15/16 of previous bar and 1/16 of current bar
    - Skips bars where no onset found within search window

    Search Window:
    ---------------
    Start: search_window_start_phase relative to 15/16 of previous bar (bar -1)
    End: search_window_end_phase relative to 1/16 of current bar (bar 0)

    Bar Skipping:
    -------------
    If no onset found within the search window at position 1/16:
    - Skip the current bar (exclude it from analysis)
    - Move to the next bar
    - Repeat the search process at the 1/16 position of that next bar
    - Continue until a valid reference onset is found
    ...
    search_window_start_phase : float
        Search window start phase relative to 15/16 of previous bar (default: 0.5)
    search_window_end_phase : float
        Search window end phase relative to 1/16 of current bar (default: 0.75)

    Returns
    -------
    tuple
        (ref_ms, ref_bar, ref_phase, grid_phase)
        Returns (0.0, first_bar, 0.0, 0.0) if no reference found

    Examples
    --------
    >>> ref_ms, bar, ref_phase, grid_phase = find_reference_offset_new_per_snippet(
    ...     downbeats, onsets, 0, 16
    ... )
    """
    # Only check tick 1 (1/16th position)
    target_tick = 1

    # Search through bars starting from first_bar
    for bar_idx in range(first_bar, len(downbeats) - 1):
        bar_start = downbeats[bar_idx]
        bar_end = downbeats[bar_idx + 1]
        bar_duration = bar_end - bar_start
        ...
        # Calculate expected grid position at 1/16th
        step_duration = bar_duration / steps_per_bar
        grid_time = bar_start + (target_tick * step_duration)

        # Calculate search window
        if bar_idx == 0:
            # For bar 0 (first bar of snippet), we can't use a previous bar
            # Search window: look before the downbeat to after the 1/16th position
            # Window start: before the downbeat (search_window_start_phase * step_duration)
            window_start = bar_start - search_window_start_phase * step_duration
            # Window end: after the 1/16th position
            window_end = grid_time + search_window_end_phase * step_duration
        else:
            # For bars > 0, use the previous bar for window calculation
            ...
            # Window start: search_window_start_phase relative to 15/16 of previous bar
            ...
            # Window end: search_window_end_phase relative to 1/16 of current bar
            window_end = grid_time + search_window_end_phase * step_duration

        # Find nearest onset within the search window
        # Filter onsets in the window
        onsets_in_window = onsets[(onsets >= window_start) & (onsets <= window_end)]

        if len(onsets_in_window) == 0:
            # No onset found in window, skip this bar and try next
            continue

        # Find onset closest to grid_time among those in the window
...
def find_reference_offset_new_4bar(
    downbeats: List[float],
    onsets: np.ndarray,
    loop_start_bar: int,
    steps_per_bar: int,
    ...
    """
    Find reference offset for 4-bar loop using NEW method.

    Similar to new_per_snippet but:
    - Uses equidistant 4-bar grid
    - ONLY checks position 1/16th of loop start bar (tick 1)
    - Uses configurable search window
    - Returns zero offset if no match found (no fallback)
    ...
    max_match_frac : float
        Maximum matching distance as fraction
    search_window_start_phase : float
        Search window start phase relative to 15/16 of previous bar
    search_window_end_phase : float
        Search window end phase relative to 1/16 of current bar
    ...
    Examples
    --------
    >>> ref_ms, bar, ref_phase, grid_phase = find_reference_offset_new_4bar(
    ...     downbeats, onsets, 0, 16
    ... )
    """
    pattern_len = 4
    ...
    # Only check tick 1 (1/16th) in the first bar of the loop
    target_tick = 1
    bar_idx = loop_start_bar

    if bar_idx == 0:
        # Cannot look at previous bar
        return 0.0, loop_start_bar, 0.0, 0.0

    # Calculate equidistant position for tick 1
    global_tick = target_tick
    grid_time = loop_start_time + (global_tick * sixteenth_duration)

    # Calculate equidistant bar boundaries for reference bar
    equi_bar_start = loop_start_time
    equi_bar_duration = steps_per_bar * sixteenth_duration

    # Get previous bar info (from actual downbeats, not equidistant)
    prev_bar_start = downbeats[bar_idx - 1]
    prev_bar_duration = downbeats[bar_idx] - prev_bar_start
    ...
    # Window end: search_window_end_phase relative to 1/16 of current bar (equidistant)
...
def find_reference_offset_new_4bar_loopstart(
    downbeats: List[float],
    onsets: np.ndarray,
    loop_start_bar: int,
    steps_per_bar: int,
    ...
    """
    Find reference offset for 4-bar loop using loop start method.

    Uses the ORIGINAL old loopwise logic:
    - Priority ticks: [0, 8, 4, 12]
    - Searches ALL bars within the 4-bar loop
    - Uses equidistant grid

    This is identical to the existing find_reference_offset_loopwise
    but fixed to 4-bar pattern length.
    ...
    Examples
    --------
    >>> ref_ms, bar, ref_phase, grid_phase = find_reference_offset_new_4bar_loopstart(
    ...     downbeats, onsets, 0, 16
    ... )
    """
    # This is the same as find_reference_offset_loopwise with pattern_len=4
    return find_reference_offset_loopwise(
        downbeats, onsets, loop_start_bar, 4, steps_per_bar, max_match_frac
    )
...
def calculate_loop_reference_offsets_new_4bar(
    downbeats: List[float],
    onsets: np.ndarray,
    bar_start_idx: int,
    bar_end_idx: int,
    ...
    """
    Calculate reference offsets for each 4-bar loop using NEW method.

    Uses find_reference_offset_new_4bar for each loop.
    ...
    for loop_start_bar in range(bar_start_idx, bar_end_idx + 1, pattern_len):
        ...
        ref_ms, bar_idx, ref_phase, grid_phase = find_reference_offset_new_4bar(
            downbeats,
            onsets,
            loop_start_bar,
            steps_per_bar
...
def calculate_loop_reference_offsets_new_4bar_loopstart(
    downbeats: List[float],
    onsets: np.ndarray,
    bar_start_idx: int,
    bar_end_idx: int,
    ...
    """
    Calculate reference offsets for each 4-bar loop using loopstart method.

    Uses find_reference_offset_new_4bar_loopstart for each loop.
    ...
    for loop_start_bar in range(bar_start_idx, bar_end_idx + 1, pattern_len):
        ...
        ref_ms, bar_idx, ref_phase, grid_phase = find_reference_offset_new_4bar_loopstart(
            downbeats,
            onsets,
            loop_start_bar,
            steps_per_bar
...
def create_comprehensive_csv(
    ...
    # Calculate NEW method phases
    print("  Calculating NEW method phases...")

    # NEW Per-Snippet method
    print("    new_per_snippet method...")
    ref_ms_new, ref_bar_new, ref_phase_new, grid_phase_new = find_reference_offset_new_per_snippet(
        downbeats, onsets, first_bar, steps_per_bar
    )
    print(f"      Reference: {ref_ms_new:.2f}ms at bar {ref_bar_new}")

    # ref_ms_new is the offset from the 1/16th position
    # calculate_phases_per_snippet shifts the entire bar by ref_ms, so we use ref_ms_new directly
    df_snippet_new = calculate_phases_per_snippet(onset_map, downbeats, ref_ms_new, steps_per_bar, remap_ticks=True)

    # NEW 4-bar method
    print("    new_4bar method...")
    loop_offsets_new_4bar = calculate_loop_reference_offsets_new_4bar(downbeats, onsets, first_bar, last_bar, steps_per_bar)
    df_loop_new_4bar = calculate_phases_loop_based(onsets, downbeats, first_bar, last_bar, 4, steps_per_bar, loop_offsets_new_4bar)
    loop_phases['new_4bar'] = (df_loop_new_4bar, loop_offsets_new_4bar, 4)

    # NEW 4-bar loopstart method
    print("    new_4bar_loopstart method...")
    loop_offsets_new_4bar_loopstart = calculate_loop_reference_offsets_new_4bar_loopstart(downbeats, onsets, first_bar, last_bar, steps_per_bar)
    df_loop_new_4bar_loopstart = calculate_phases_loop_based(onsets, downbeats, first_bar, last_bar, 4, steps_per_bar, loop_offsets_new_4bar_loopstart)
    loop_phases['new_4bar_loopstart'] = (df_loop_new_4bar_loopstart, loop_offsets_new_4bar_loopstart, 4)
    ...
    # Merge NEW per-snippet
    df_snippet_new_merge = df_snippet_new[['bar_number', 'tick_16th', 'phase_per_snippet_remapped']].copy()
    df_snippet_new_merge['bar_number'] = df_snippet_new_merge['bar_number'] - first_bar
    df_snippet_new_merge = df_snippet_new_merge.rename(columns={'phase_per_snippet_remapped': 'phase_new_per_snippet'})
    df_comprehensive = df_comprehensive.merge(df_snippet_new_merge, on=['bar_number', 'tick_16th'], how='left')
    ...
    # NEW per-snippet reference
    if ref_bar_new is not None and ref_bar_new >= first_bar:
        # Calculate bar duration for coordinate transformation
        bar_duration_new = 0.0
        if ref_bar_new < len(downbeats) - 1:
            bar_duration_new = downbeats[ref_bar_new + 1] - downbeats[ref_bar_new]

        ref_rows.append({
            'method': 'new_per_snippet',
            'bar_number': ref_bar_new - first_bar,
            'bar_number_global': ref_bar_new,
            'ref_ms': ref_ms_new,
            'ref_phase': ref_phase_new,
            'grid_phase': grid_phase_new,
            'bar_duration': bar_duration_new
        })
