...
def create_rhythm_histograms_with_style(output_dir: Path):
    """
    Create rhythm histograms with style using filtered flexStart CSVs.
    ...
    """
    print("\n" + "=" * 80)
    print("RHYTHM HISTOGRAMS WITH STYLE (Filtered FlexStart)")
    print("=" * 80)
    ...
    methods = [
        ('Per-Snippet L=4', 4),
        ('Per-Snippet L=2', 2),
        ('FlexStart Pattern Length 4', 4),
        ('FlexStart Pattern Length 2', 2),
        ('FlexStart Pattern Length 1', 1),
    ]

    # Initialize histogram accumulators
    histograms = {}
    pattern_counts = {}
    for method_name, pattern_length in methods:
        histograms[method_name] = np.zeros(pattern_length * 16)
        pattern_counts[method_name] = 0

    # Accumulate histograms from all tracks (read from 5.5_rhythm folder CSV files)
    tracks_processed = 0
    for track_dir in track_dirs:
        rhythm_csv = track_dir / '5.5_rhythm' / f'{track_dir.name}_rhythm_histograms_with_style.csv'
            ...
            try:
                ...
                for method_name, pattern_length in methods:
                    ...
                    if len(method_data) > 0:
                        ...
                        # Track pattern count (all rows should have same num_patterns for this track+method)
                        # We add the count once per track
                        if 'num_patterns' in method_data.columns:
                            num_patterns = method_data['num_patterns'].iloc[0]
                            if pd.notna(num_patterns):
                                pattern_counts[method_name] += int(num_patterns)
    ...
    fig, axes = plt.subplots(5, 1, figsize=(16, 20))
    fig.suptitle(f'Rhythm Histograms (Filtered FlexStart) — Onset Distribution Across {tracks_processed} Tracks',
                 fontsize=16, fontweight='bold')
    ...
    # Create histograms for each method
    for idx, ((method_title, pattern_length), color) in enumerate(zip(methods, colors)):
        ax = axes[idx]
        ...
        ax.set_ylabel('Onset Strength', fontsize=11, fontweight='bold')
        ...
        ax2 = ax.twinx()
        ax2.set_ylabel('Onset Count', fontsize=11, fontweight='bold', rotation=270, labelpad=15)
        ...
        total_patterns = pattern_counts[method_title]
        title = f'{method_title} (L={pattern_length}, {num_positions} positions) — {total_patterns} patterns'
        ...
        print(f"  Total onsets: {total_onsets}")
        print(f"  Occupied positions: {occupied_positions}/{num_positions}")
        print(f"  Max count at one position: {max_count_stat}")
        print(f"  Total patterns: {total_patterns}")

    plt.tight_layout()

    # Save figure
    output_file = batch_dir / 'rhythm_histograms_with_style.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"\n✓ Saved rhythm histograms with style: {output_file}")

    # Also save as PDF
    output_pdf = batch_dir / 'rhythm_histograms_with_style.pdf'
    plt.savefig(output_pdf, bbox_inches='tight')
    print(f"✓ Saved rhythm histograms with style: {output_pdf}")
...
if __name__ == '__main__':
    ...
    # Create both types of rhythm histograms
    create_rhythm_histograms(output_dir)
    create_rhythm_histograms_with_style(output_dir)
